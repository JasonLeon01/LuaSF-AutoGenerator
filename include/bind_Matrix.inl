    ...
    type.set_function(
        ...
        sol::factories(
            [](sol::object values) {
                ...
                if (buffer.size() != N * N)
                    throw std::runtime_error("matrix constructor expects exactly N * N float values");
    ...
    type.set(
        ...
        sol::property(
            ...
            [](Matrix& self, sol::object values) {
                auto buffer = array_from_object<float>(values);
                if (buffer.size() != N * N)
                    throw std::runtime_error("matrix array assignment expects exactly N * N float values");
                std::copy(buffer.begin(), buffer.end(), self.array.begin());
            }));

    type.set_function("copyMatrix", [](const sf::Transform& source, Matrix& dest) { sf::priv::copyMatrix(source, dest); });

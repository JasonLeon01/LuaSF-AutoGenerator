...
inline bool is_doc_path_prefix(char value) {
    return is_doc_command_char(value) || (value >= '0' && value <= '9') ||
           value == '_' || value == ':' || value == '/' || value == '\\';
}

inline bool is_doxygen_command(std::string_view value) {
    static constexpr std::array<std::string_view, 21> commands = {
        "a",     "b",       "brief",   "c",   "code",   "deprecated", "e",
        "em",    "endcode", "ingroup", "li",  "note",   "overload",   "p",
        "param", "relates", "return",  "see", "throws", "warning"};
...
inline std::string normalize_doc_line(const std::string& line) {
    ...
    for (std::size_t index = 0; index < line.size();) {
        if (line[index] != '\\' ||
            ...
            index + 1 >= line.size() || !is_doc_command_char(line[index + 1])) {
        ...
        const std::string_view command(line.data() + index + 1,
                                       end - index - 1);
...
inline FunctionType parse_function_type(const std::string& value) {
    ...
    for (const std::string& parameter :
        ...
        result.parameters.push_back(
            {std::move(name), parameter_type, optional});
    }
...
inline void flush_pending_function() {
    ...
    // Capture and clear before any call that may re-enter
...
inline void doc(std::string_view text) {
    if (!enabled() || text.empty()) {
        return;
    }

    // A docstring belongs to the declaration emitted by the following stub
    // macro. Flush a preceding function first, but retain a pending class so
    // this can also annotate its fields.
    flush_pending_function();
    pending_doc() = text;
}

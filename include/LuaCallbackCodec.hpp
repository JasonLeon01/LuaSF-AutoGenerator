...
LUASF_API std::shared_ptr<CallbackContext> makeCallbackContext(
...
struct FromObject<sf::SoundSource::EffectProcessor,
    ...
    static sf::SoundSource::EffectProcessor read(const lua_glue::Object& object,
                                                 CallbackOptions options) {
        const auto context =
            makeCallbackContext(object, std::move(options.label));
        return
            [context](const float* inputFrames, unsigned int& inputFrameCount,
                      float* outputFrames, unsigned int& outputFrameCount,
                      unsigned int frameChannelCount) noexcept {
                invokeInterleavedFloatTransform(
                    context, inputFrames, inputFrameCount, outputFrames,
...
struct FromObject<sf::Text::GlyphPreProcessor, GlyphPreProcessorCodec> {
    static sf::Text::GlyphPreProcessor read(const lua_glue::Object& object,
                                            CallbackOptions options) {
        const auto context =
            makeCallbackContext(object, std::move(options.label));
        return
            [context](const sf::Text::ShapedGlyph& glyph, std::uint32_t& style,
                      sf::Color& fill, sf::Color& outline, float& thickness) {
                invokeGlyphPreProcessor(context, glyph, style, fill, outline,
                                        thickness);
            };
    }
};
...
struct FromObject<std::function<bool(const void*, std::size_t)>,
    ...
    static std::function<bool(const void*, std::size_t)> read(
        const lua_glue::Object& object, CallbackOptions options) {
        const auto context =
            makeCallbackContext(object, std::move(options.label));
...
struct FromObject<std::function<bool(void*, std::size_t&)>,
    ...
    static std::function<bool(void*, std::size_t&)> read(
        const lua_glue::Object& object, CallbackOptions options) {
        const auto context =
            makeCallbackContext(object, std::move(options.label));
...
struct FromObject<std::function<void(Arguments...)>,
    ...
    static std::function<void(Arguments...)> read(
        const lua_glue::Object& object, CallbackOptions) {
...
template <typename NativeCallable, typename Codec>
NativeCallable from_object(const lua_glue::Object& object,
                           CallbackOptions options = {}) {
    const bool isNil = !object.valid() || object == lua_glue::nil;
    if (isNil) {
        if (options.allowNil) {
            return NativeCallable{};
        }
        throw std::invalid_argument(
            options.label.empty() ? "Lua callback does not allow nil"
...
template <typename NativeCallable, typename Codec>
lua_glue::Object to_object(lua_glue::StateView, const NativeCallable&,
                           CallbackOptions = {}) {
...
template <typename NativeCallable, typename Codec>
lua_glue::Object to_object(lua_glue::StateView lua,
                           const NativeCallable& callable,
                           std::string_view label) {

...
try:
    from .output_files import copy_file_if_changed, write_text_if_changed
...
def copy_support_sources(project_root: Path, output_root: Path) -> None:
    ...
    for path in include_out.glob("*"):
        if (path.is_file()
                and (path.name in {"utils.hpp", "utils.inl"} or path.name.startswith("bind_"))
                and not (project_root / "include" / path.name).is_file()):
            path.unlink()
    for path in src_out.glob("bind_*.cpp"):
        if path.is_file() and not (project_root / "src" / path.name).is_file():
            path.unlink()
...
def remove_legacy_generated_layout(project_root: Path, output_root: Path) -> bool:
    """Migrate only the generator's original, positively identified output layout."""
    if output_root.resolve() != (project_root / "output" / "LuaSF").resolve():
        return False
    legacy_root = output_root.parent
    entrypoint = legacy_root / "LuaSF.cpp"
    wrapper = legacy_root / "include" / "luasf_sol.hpp"
    marker = "// This file is generated by tools/generate_build_files.py. Do not edit manually."
    if (entrypoint.is_symlink() or wrapper.is_symlink()
            or not entrypoint.is_file() or not wrapper.is_file()):
        return False
    if not entrypoint.read_text(encoding="utf-8").startswith(marker + "\n"):
        return False
    if not all((output_root / name).is_file()
               for name in ("CMakeLists.txt", "LuaSF.cpp", "include/LuaSF.hpp")):
        return False

    for name in ("CMakeLists.txt", "LuaSF.cpp", "binding_order.json",
                 "callback_codecs.json", "sfml_api.json", "include", "src", "tools"):
...
def save_cached_order(
    ...
    write_text_if_changed(cache_path,
        json.dumps(
            {
...
def render_outputs(
    ...
    write_text_if_changed(cmake_output,
    ...
    write_text_if_changed(public_header_output,
    ...
    write_text_if_changed(source_output,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--cache", default="output/LuaSF/binding_order.json")
    ...
    parser.add_argument("--cmake-output", default="output/LuaSF/CMakeLists.txt")
    parser.add_argument("--source-output", "--main-output", default="output/LuaSF/LuaSF.cpp", dest="source_output")
    parser.add_argument("--header-output", default="output/LuaSF/include/LuaSF.hpp")
    parser.add_argument("--callback-codecs-output", default="output/LuaSF/callback_codecs.json")
    ...
    parser.add_argument("--force-sort", action="store_true", help="Ignore output/LuaSF/binding_order.json and recompute the order.")
...
def main() -> int:
    ...
    glue_root = project_root / "LuaGlue"
    if not (glue_root / "CMakeLists.txt").is_file():
        raise FileNotFoundError("LuaGlue/CMakeLists.txt is required for source distribution")
    shutil.copytree(glue_root, output_root.parent / "LuaGlue", dirs_exist_ok=True,
                    copy_function=copy_file_if_changed,
                    ignore=shutil.ignore_patterns("build", ".git", "__pycache__"))
    copy_file(project_root / "LICENSE", output_root / "LICENSE")
    ...
    if remove_legacy_generated_layout(project_root, output_root):
        print("Removed the previous generated layout from output/.")

    cache_note = "using cached order" if used_cache else "after topological sort"
    print(
        f"Generated output/LuaSF/CMakeLists.txt, output/LuaSF/LuaSF.cpp, output/LuaSF/include/LuaSF.hpp, "
        f"and output/LuaSF/callback_codecs.json "

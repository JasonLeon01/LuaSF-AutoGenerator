...
DEPENDENCY_DIR="$SCRIPT_DIR/third_party"
CONFIG=${1:-}
...
EMBEDDED_MODULE_FILE=$(
    find "$EMBEDDED_BIN_DIR" -maxdepth 1 \( -type f -o -type l \) \( -name 'LuaSF.dll' -o -name 'LuaSF.dylib' -o -name 'LuaSF.so' \) 2>/dev/null | head -n 1 || true
)
